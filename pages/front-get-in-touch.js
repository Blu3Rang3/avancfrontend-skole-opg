...
export default function FrontGetInTouch() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        // Fetch image
        const imageResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/images/front-getintouch-link.png`);
        if (!imageResponse.ok) {
        ...
        // Fetch title snippet
        const titleResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/text-snippets/66bb28352f4463f4250d8660`);
        if (!titleResponse.ok) {
          throw new Error('Error fetching title snippet');
        }
        const titleData = await titleResponse.json();
        console.log('Title Data:', titleData); // Log the data for debugging
        ...
        console.log('Content Data:', contentData); // Log the data for debugging
