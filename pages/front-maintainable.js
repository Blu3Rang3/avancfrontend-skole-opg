...
export default function FrontMaintainable() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        // Fetch image
        const imageResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/images/front-maintainable-link.png`);
        if (!imageResponse.ok) {
        ...
        // Fetch title snippet
        const titleResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/text-snippets/66bb28352f4463f4250d865e`);
        if (!titleResponse.ok) {
  ...
  return (
    <RootLayout>
      <div className="flex items-center justify-center min-h-screen bg-skyBlue">
